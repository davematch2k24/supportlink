...
const statuses = ['Completed', 'Rejected', 'Processing']
...
const updateStatus = async (status) => {
  ...
  resultData.value.req_status = status
}
            ...
            <v-col cols="12" md="8">
              <v-sheet class="mx-auto py-1 px-4" elevation="3" style="max-width: 800px">
                <div class="info-section">
                  <p><strong>Full Name:</strong> {{ resultData.name }}</p>
                  <p><strong>Phone Number:</strong> {{ resultData.contact_number }}</p>
                  <p><strong>Email Address:</strong> {{ resultData.email }}</p>
                  <p><strong>Home Address:</strong> {{ resultData.address }}</p>
                  <p>
                    <strong>Number of Family Members:</strong>
                    {{ resultData.family_count }}
                  </p>
                </div>
                <v-divider></v-divider>
                <div class="info-section">
                  <p><strong>Request Type:</strong> {{ resultData.req_type }}</p>
                  <p><strong>Request Purpose:</strong> {{ resultData.req_purposes }}</p>
                </div>
                <v-divider></v-divider>
                <div class="info-section">
                  <p class="request-status">
                    <strong>Request Status:</strong>
                    <span :class="(resultData.req_status || 'pending').toLowerCase()">
                      <span class="circle"></span> {{ resultData.req_status || 'Pending' }}</span
                    >
                  </p>
                </div>
                <v-row>
